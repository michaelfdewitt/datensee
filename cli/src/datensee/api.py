...
def _now_micros() -> int:
    ...
    (not nanoseconds) to match EE's ``version`` load argument;
    see :mod:`datensee.pinning`.
    """
...
# installed client; when they hand us a string we pass it through.
# ---------------------------------------------------------------------------
...
# Catches forgetting to reduce a collection before exporting.
...
def export(
    ...
    *,
    ...
    Args:
        ...
        output: Output path: GCS URI (gs://…) for Dataflow, or local directory.
        scale: Pixel size in meters.
        crs: Target CRS (EPSG code or proj string).
        tile_size: Compute tile edge size in pixels (sent to the EE HV API).
        output_tile_size: Output COG edge size in pixels for two-tier mode.
            Must be a positive multiple of ``tile_size``. When None, one COG
            is written per compute tile. When set to a multiple > tile_size,
            compute tiles are assembled into larger COGs.
        nodata: Optional nodata value stamped on every output COG as the
            ...
            channel: unmask(sentinel) the expression and pass the sentinel
            here so GIS tools distinguish nodata from real zeros.
        band_count: Bands your expression produces (default 1).
        ...
        labels: Dataflow job labels, forwarded as additionalUserLabels.
        jar: Path to the pipeline JAR (auto-detected if None).
        snapshot_time: Unix microseconds to pin every asset reference in
            ``ee_expression``. Defaults to current timestamp at submission.
        dry_run: If True, validate but don't submit.
        progress_callback: Optional callback(completed, total) for local progress.
        confirm_callback: Optional callback(PipelineConfig) before submission.
            Raise an exception (e.g. ``typer.Abort``) to cancel.
        credentials: Optional caller-supplied Google credentials. When set,
            DatensEE skips its normal ADC bootstrap and uses these credentials
    ...
    # Save user expression for the meta sidecar before clipping.
    ee_expression_user = ee_expression

    # Pin asset loads to a single snapshot time.
    ...
    # Per-tile grids already specify the exact pixels EE computes; full-region
    # polygon evaluation per tile adds redundant overhead.
    # Tile: derive the grid from region+scale, or tile the caller grid verbatim.
    ...
    # Build config: worker-pool kwargs are optional; override only if set.
    if runner == "dataflow":
    ...
    # Persist metadata before submission so retries can verify configuration.
    ...
    # For local mode: count files produced for this specific run.
    tiles_ok: int | None = None
    tiles_failed: int | None = None
    ...
    *,
    ...
    Args:
        ...
        watchdog: :class:`datensee.status.WatchdogConfig` controlling
            ...
            idle_timeout). Defaults to package-level defaults (see
...
class RetryResult(BaseModel):
    ...
    """The project the round ran under, resolved from the meta sidecar."""
...
def retry(
    *,
    ...
    Reads ``journal`` (NDJSON ``_failures.json``), classifies each entry, and
    emits quadtree sub-tiles for split-eligible errors (or re-emits the same
    tile for transient errors).

    The ``{output}/_export_meta.json`` sidecar is required to anchor journal
    pixel offsets to the original parent PixelGrid.

    Every retry round runs with ``merge_existing_output`` enabled: the
    pipeline overlays re-fetched tiles onto existing COGs, preserving
    previously-successful pixels.

    Args:
        output: Output path of the original export.
        journal: Path to the failures journal (NDJSON). Defaults to
            ``{output}/_failures.json``.
        ee_expression: Expression to evaluate. Defaults to value in meta.
        project: GCP project. Defaults to value in meta.
        scale, crs, tile_size, output_tile_size, nodata: Export grid
            parameters. Defaults to values in meta.
        max_depth: Max quadtree depth. Records at this depth remain in
            failures without splitting. Default 2.
        dry_run: If True, plan the retry but do not submit.
        credentials: Optional Google credentials for API calls.

    Returns:
        RetryResult with job ID and breakdown statistics.
    """
    import logging
    ...
    # Resolve shape args against persisted meta.
    ...
    # unpinned; we fall back to fresh T and warn.
    from datensee.pinning import pin_expression
    ...
    if persisted_meta is not None and persisted_meta.snapshot_time is not None:
        ...
        logging.getLogger(__name__).warning(
            ...
            "unpinned. Retry children will pin to %d (now). Output COGs "
            "may mix sub-tiles fetched at different snapshots.",
            snapshot_time_micros,
        )
    ee_expression = pin_expression(ee_expression, snapshot_time_micros)

    # The retry's parent PixelGrid must match the original export.
    ...
    pipeline_config = PipelineConfig(
        ...
        output=OutputConfig(
            output_path=output,
            output_tile_size_pixels=output_tile_size,
            # Retry overlays re-fetched tiles onto existing output COGs.
    ...
    # For local mode: count failures from this specific round.
    tiles_failed_this_round: int | None = None
...
class RetryUntilDoneResult(BaseModel):
    ...
    ``stopped`` explains why the loop ended:
    * "no_retryable_work": journal holds nothing actionable by policy.
    * "max_rounds": round budget exhausted with retryable work remaining.
    * "job_<STATE>": Dataflow round ended in non-DONE terminal state.
    """
...
def retry_until_done(
    ...
    **retry_kwargs: Any,
    ...
    maintains with fresh failures and carryover), submits actionable work,
    and stops when a round has nothing to submit or the round limit is reached.
    A short backoff separates rounds.
    """
