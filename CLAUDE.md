...
10. **M10: Snapshot Pinning** ✅ — Every export captures a single Unix-nanos timestamp `T` at submit and rewrites every asset-load node in the serialized EE expression to pin its `version` arg to `T`. All workers see one snapshot, so a mutating ImageCollection can't let tile A see the new version while tile B sees the old one. `T` is stored in `_export_meta.json` and inherited by `datensee retry` so split children land on the same snapshot as their parents. BigQuery loads can't be pinned through this mechanism: `runBigQuery` is rejected unless its SQL contains `FOR SYSTEM_TIME AS OF`, and `loadBigQuery` is unconditionally rejected. Walker lives in [`cli/src/datensee/pinning.py`](cli/src/datensee/pinning.py); `--snapshot-time` (ISO-8601 or nanos) is the user-facing override.
11. **M11: PixelGrid as canonical export shape** — Every export normalizes to a single `PixelGrid` (`{crs_code, affine_transform, dimensions}`) that mirrors EE's own type, sent verbatim to `computePixels`. Tiles inside the export become integer pixel rectangles; float bboxes are derived from `transform × pixel_offsets` and never persisted. `scale_meters` for geographic CRSs uses the equator constant (no latitude correction), so cross-export grid alignment is unconditional — same CRS + same scale + same tile size = identical pixel-to-CRS transform regardless of region. Closes the geographic-CRS footgun M10 introduced. **Two-session plan in [`docs/m11-pixel-grid.md`](docs/m11-pixel-grid.md)** — Session A is Python only (intermediate state has Java tests broken; documented), Session B is Java + Java tests.
