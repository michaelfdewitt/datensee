...
Cloud Build builds and pushes the launcher image from a source tarball
(``pipelines/Dockerfile`` + the shadow JAR), then the template spec JSON is
written the way ``gcloud dataflow flex-template build`` writes it. That is
all Dataflow needs. The JAR users download comes from the GitHub Release,
built and pinned by ``.github/workflows/publish.yml``, never from here.
Needs only Application Default Credentials with Cloud Build + Artifact Registry
+ GCS permissions on the hosting project; it runs from a bare container or CI.
...
Build the JAR first (``cd pipelines && ./gradlew shadowJar``). Environment
overrides: ``DATENSEE_TEMPLATE_PROJECT``, ``_REGION``, ``_REPO``, ``_BUCKET``.
"""
