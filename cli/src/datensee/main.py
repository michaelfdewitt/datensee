...
def _load_expression_and_region(
    ...
    ``{"expression": {...}, "region": {...}}`` (as output by the Code Editor
    snippet). A bundle plus a separate region file is redundant and rejected.
    """
    ...
    nodata: Annotated[
        ...
        typer.Option(
            ...
            help=(
                ...
                "Earth Engine returns masked pixels as 0; unmask(sentinel) your "
                "expression and pass the sentinel here so GIS tools can "
                "distinguish nodata from valid zero values."
            ),
        ),
    ] = None,
    runner: Annotated[
...
def status(
    ...
    failure-rate breaker, and idle timeout) that cancel the underlying
    Dataflow job if breached. Each policy is independently configurable
    via flags above; ``--no-watchdog`` disables all watchdog controls.
    """
...
def validate_cmd(
    ...
    Two checks are supported: ``integrity`` (zero-cost verification that
    every expected output COG exists or is journaled, with correct dimensions,
    CRS, and origin) always runs; ``--pixels`` additionally re-fetches sampled
    tiles from the High Volume API and compares all bands.
    """
    from datensee.pixel.validation import validate_output
    ...
    if json_output:
        ...
        console.print(f"Validation report written to {json_output}")
    else:
        console.print(report.render())
...
def retry_cmd(
    output: Annotated[
        ...
        typer.Option(
            "--output",
            "-o",
            help="Output path of the original export (locates the meta "
            "sidecar and failures journal).",
        ),
    ],
    expression_file: Annotated[
        Path | None,
    ...
    if until_done:
        ...
        if loop_result.stopped == "no_retryable_work":
            if last.carryover_count == 0:
                ...
                console.print(
                    f"[yellow]{last.carryover_count} record(s) remain incomplete "
                    f"(terminal or depth-capped) after {len(loop_result.rounds)} "
                    "round(s). Inspect _failures.json; increasing --max-depth may "
                    "allow further retries.[/yellow]"
                )
        elif loop_result.stopped == "max_rounds":
            console.print(
                f"[yellow]Round limit ({max_rounds}) reached with retryable "
