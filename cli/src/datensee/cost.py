...
Deterministic calculation over :class:`~datensee.config.PipelineConfig`
without network I/O. Produces a :class:`CostEstimate` covering three cost
surfaces:

* **EECU:** Earth Engine compute units consumed by High Volume API requests.
  Free for non-commercial EE projects; commercial plans bill per EECU-hour.
* **Dataflow:** Worker vCPU and memory hours (plus Shuffle in two-tier mode).
* **GCS storage:** Monthly storage cost of output COGs.

Model constants are defined below. Estimates represent approximate ranges;
EECU consumption depends on the underlying Earth Engine expression complexity.
"""
...
# ---------------------------------------------------------------------------
# Pricing and model constants (verified against 2026-07 GCP list prices).
#
# Dataflow and GCS rates use batch / us-central1 list prices.
# ---------------------------------------------------------------------------

# EE compute: per-tile EECU-seconds range (expression-dependent).
...
class CostEstimate:
    ...
    Attributes:
        ...
        assumptions: Statements of modeling assumptions applied.
    """
...
def estimate_cost(config: PipelineConfig, *, tile_count: int | None = None) -> CostEstimate:
    """Estimate EECU, Dataflow, and storage cost for an export configuration.

    Calculates estimated usage and costs from configuration parameters. See
    constants at the top of this module for pricing rates.
