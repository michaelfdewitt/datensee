"""The ``pixels`` check — re-fetch sampled tiles from EE and compare.

The terminal check: re-fetch sampled non-journaled compute tiles from
the EE HV API in NPY format and compare against the pipeline output on
disk. The request grid is built exactly like the Java fetcher's
``PixelGrid.forTile``: parent scale verbatim, translate shifted by the
tile's pixel offset, the tile's own dimensions. In two-tier mode the
on-disk data for a compute tile is a window of its output COG.

Opt-in via ``validate_output(..., pixels=True)`` — costs EECUs and
requires an EE-enabled GEE project plus rasterio.
"""
...
def _build_hv_request(expression: str, grid: PixelGrid, file_format: str = "NPY") -> dict[str, Any]:
    """Build a computePixels request body — the per-tile grid sent verbatim."""
...
def _read_disk_pixels(
    ...
    """All bands of the tile's on-disk data — a COG window in two-tier mode."""
    import rasterio
