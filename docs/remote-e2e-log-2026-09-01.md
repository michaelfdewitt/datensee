...
## Phase 6 — tabula rasa drill (fresh user, release wheel)

- [SETUP] `uv build` on the branch → datensee-0.1.0a3-py3-none-any.whl (the identical artifact publish.yml uploads). New Linux user `tabula` with a pristine HOME: no repo checkout, no JAR cache, no GITHUB_TOKEN — only the ADC file and the system JRE/uv/python3.
- [PASS] As tabula: wheel install → `datensee 0.1.0a3`; `jar download` with empty cache and no token → bucket fallback, sha256 verified; `demo` → 9 tiles in 10 s; `validate ~/demo-local` → integrity PASS (rasterio-less degradation noted in message); `export --runner dataflow` on the DEFAULT v0.1.0a3 spec → job 2026-09-01_14_39_01-13868086705965018493 submitted (us-east1).
- [PASS] Cloud leg as tabula: job 2026-09-01_14_39_01-13868086705965018493 → DONE in 4.8 min, tracked by tabula's own `datensee status` ("Job completed successfully"); `validate gs://…/tabula-20260901T213858Z` (default config) → integrity PASS; all 9 cloud tiles byte-identical to tabula's local run.
- [CONCLUSION] The 0.1.0a3 release wheel works from a pristine account with nothing but ADC and a JRE: install → jar download (bucket fallback, sha verified) → local demo → validate → Dataflow on the default spec → status → validate gs://. The only delta from a real PyPI user is the wheel arriving by file copy instead of `pip download` — the artifact is the same one publish.yml uploads.

## Phase 7 — hardening + scale (2026-09-02)

- [FIX] Wheel-pinned JAR digest (user's design): `cli/src/datensee/_jar_digest.py` bakes the release JAR's sha256 into the wheel; `jar.py` verifies EVERY download against it (GitHub asset AND bucket — the GitHub path was previously unverified, and release assets are mutable via --clobber). Hosts are now availability only; trust root is PyPI. Release scripts write the pin (commit with the release); publish.yml's jar job no longer rebuilds — it downloads the bucket-staged artifact, verifies against the committed pin, attaches those exact bytes to the GitHub Release, and fails loudly if the template was never staged. Live check on the LXC (no token, fresh HOME): "sha256 verified against the release pin."
- [SECURITY] /security-review on the branch: no HIGH/MEDIUM findings; its advisory (same-origin sidecar = integrity, not authenticity) is what the digest pin closes.
- [PASS-SCALE] 20°×20° SRTM export, 21,316 tiles (512 px), tiles_file externalization (first Dataflow outing): job 2026-09-02_05_43_59-6816905880710455580 (us-east1, e2-standard-4, 2–16 workers) DONE in 10 min 14 s wall. tiles_written=21,316.
