...
- Adaptive splitting is opt-in: this module isn't called from the
  ...
  gate. Splitting works for every export shape — retry rounds run with
  ``merge_existing_output``, so split children overlay their parent's
  output COG in place regardless of two-tier configuration.
"""
...
def plan_retry(
    ...
    *,
    ...
    ``unknown_kind`` — so the pipeline can union them into the next
    ``_failures.json`` and a downstream reader can tell at a glance why
