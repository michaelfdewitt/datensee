 ...
 *
 * <p>The default Dataflow flow runs workers under a service account that
 * the user must grant {@code roles/earthengine.viewer} (or equivalent) on
 * the EE project. First-run users hit a wall of 403s from the HV API
 * with no signal about which knob to turn. This helper produces a
 * specific gcloud invocation so the next run works.
 */
public final class EeAuthRemediation {
     ...
     * Direct runner on a developer laptop) — callers must tolerate it.
     */
