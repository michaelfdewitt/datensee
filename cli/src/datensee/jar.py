...
def _no_release_message(version: str) -> str:
    return (
        f"No prebuilt pipeline JAR for v{version}: no asset on the GitHub Release "
        f"(https://github.com/{GITHUB_REPOSITORY}/releases/tag/v{version}).\n"
        "Options:\n"
...
def download_jar(version: str = __version__, *, force: bool = False) -> Path:
    ...
    The JAR is the asset attached to the GitHub Release for tag
    ``v<version>``. When ``version`` is the installed package's own
    version, the bytes are verified against the digest baked into this
    wheel at release time; any other version is downloaded as published.
    ...
    if pinned is not None:
        ...
        console.print(
            "  [yellow]Not verified: this datensee release pins only its own JAR "
            f"version; v{version} is downloaded as published.[/yellow]"
        )
...
def _pinned_digest(version: str) -> str | None:
    ...
    of any *other* version has no trustworthy local pin and is fetched
